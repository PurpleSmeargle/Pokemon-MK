...
module Input
  ...
  def self.triggere?(sym)
    if Input.press?(sym)
      p 'pressing'
      return false
    end
    sym = sym.to_s.upcase.to_sym
    codes = []
    case sym
    when :CANCEL
      codes << :X
    ...
    codes.map! { |key| Scancodes::SDL[key] }
    codes.each do |code|
      if MKXP.raw_key_states.getbyte(code) == 1 && !@code_timeout[code]
        codes.each do |code|
          @code_timeout[code] = 20
        end
        return true
      end
    end
  ...
  def self.updatee
    old_update
    @code_timeout.keys.each do |key|
      @code_timeout[key] -= 1 if @code_timeout[key].is_a?(Numeric)
      @code_timeout.delete(key) if @code_timeout[key] <= 0
    end
  end
  ...
  def self.presse?(sym)
    ...
    codes.map! { |key| Scancodes::SDL[key] }
    codes.each do |code|
      return true if MKXP.raw_key_states.getbyte(code) == 1
    end
    return false
  end
end
